...
import { V4Header } from "@/app/components/V4Header";

interface V4ProductPageProps {
...
async function getProductData(id: string) {
  "use cache: remote";
  cacheLife("days");
  cacheTag(`v4-product-${id}`);

  return { cachedAt: new Date() };
}

async function V4ProductContent({ id }: { id: string }) {
  ...
  return (
    <div className="px-6 py-8">
      ...
      <div className="mt-6 text-sm text-zinc-600 dark:text-zinc-400">
        <p>Cache tag: <code className="bg-zinc-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded text-xs">v4-product-{id}</code></p>
      </div>
    </div>
  );
}

export default async function V4ProductPage({ params }: V4ProductPageProps) {
